...
//    if ([self tag] === 2306)
        CPLog.trace("KEY return="+((aString === CPNewlineCharacter || aString === CPCarriageReturnCharacter))+" state="+[self themeState]);
}
...
- (void)encodeWithCoder:(CPCoder)aCoder
{
    ...
    if (_title === @"TOTO")
        CPLog.info("=============== POSITION="+[self imagePosition]);

    CPLog.info("BEFORE KEY:"+_title);
    if (_keyEquivalent)
    {
        [aCoder encodeObject:CFData.encodeBase64Utf16String(_keyEquivalent) forKey:CPButtonKeyEquivalentKey];
        CPLog.info("INSIDE KEY="+_keyEquivalent+"*");
    }
    CPLog.info("AFTER KEY");
