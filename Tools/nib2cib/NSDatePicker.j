...
- (CGRect)_nib2CibAdjustment
{
    ...
    if ([self tag] > 200)
        CPLog.info([self tag]+":isClockOnly="+isClockOnly+" adjustment="+CPStringFromRect(frameAdjustment)+" frame="+CPStringFromRect([self frame]));
...
- (void)_adjustNib2CibSize
{
    ...
    if (frameAdjustment)
    {
        ...
        if ([self tag] > 200)
            CPLog.info([self tag]+":final frame="+CPStringFromRect(finalFrame));
